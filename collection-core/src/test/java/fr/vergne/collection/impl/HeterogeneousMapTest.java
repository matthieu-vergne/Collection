 ...
 * @deprecated Deprecated since the tested class is deprecated. See the
 *             documentation of {@link HeterogeneousMap} for details.
 */
public class HeterogeneousMapTest {
	...
	public void testKeysInEntrySetCorrespondToKnownKeys() {
		...
		List<?> keys = entrySet.stream().map((entry) -> entry.getKey()).collect(Collectors.toList());
	...
	public void testValuesInEntrySetCorrespondToKnownValues() {
		...
		Set<Entry<Key<?>, Object>> entrySet = map.entrySet();
		List<Object> values = entrySet.stream().map((entry) -> entry.getValue()).collect(Collectors.toList());
	...
	public void testDerivedMapsDoNotEqualTheirHeterogeneousParent() {
		...
		/*
		 * Usual maps are not equal to instances of our HeterogeneousMap, because it is
		 * not a usual map. Thus, we must ensure that the derived map remains consistent
		 * with this fact.
		 */
		assertFalse(map.toMap().equals(map));
		/*
		 * Because equal() must be symmetric, we must ensure that it is also the case
